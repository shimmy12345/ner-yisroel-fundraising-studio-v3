...
async function run() {
  ...
  assert.match(relationshipSummary.content, /Imported context \(not verified, not counted as contact\)/, "relationship-summary must also surface unconfirmed context, never silently merged into the summary text above it");
  ...
  assert.doesNotMatch(noUnconfirmed.content, /Imported context \(not verified/, "the block must be omitted entirely when there is nothing unconfirmed, not shown empty");
