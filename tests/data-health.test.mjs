...
function fictionalFailureDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE interactions (id TEXT PRIMARY KEY,donor_id TEXT,user_id TEXT);
    CREATE TABLE recommendations (id TEXT PRIMARY KEY,donor_id TEXT,user_id TEXT);
  ...
  db.prepare("INSERT INTO interactions VALUES (?,?,?)").run("interaction-orphan", "missing", owner);
  db.prepare("INSERT INTO recommendations VALUES (?,?,?)").run("reminder-orphan", "missing", owner);
...
test("six-month edge cases avoid false green results", () => {
  ...
  assert.equal(inferMigrationLevel(["donors","data_imports","donor_views","relationship_queue_dismissals"],[],[],[]),"0017");
