...
// Regression coverage for Design C: latest/'s own R2 object metadata
...
// pattern-matching alone (see tests/backup-automation.test.mjs for that
// style, used here only for the mechanical, low-risk parts).
...
async function run() {
  ...
  assert.match(monthly, /verifiedBackupObjectKey: null, verifiedBackupCompletedAt: null/, "the published fallback record must carry explicit nulls, never a guessed identity");
  assert.match(monthly, /verifiedBackupObjectKey: \$backupKey, verifiedBackupCompletedAt: \$backupAt/, "the published known-identity record must carry both fields together");
  ...
  // publish step -- this is what resolves the conflict Design A had with
  // this exact invariant (Design C never needs the status-bucket
  // credential to determine identity at all).
