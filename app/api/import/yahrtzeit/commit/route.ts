...
// duplicating them. Never touches giving_activities, gifts, interactions,
// or recommendations -- the only tables this route writes are yahrtzeits
// and its own audit trail, yahrtzeit_changes.
...
export async function POST(request: Request) {
  ...
  for (const row of preview) {
    ...
    if (existing) {
      ...
      const changedFields = (Object.keys(after) as (keyof typeof after)[]).filter((key) => before[key] !== after[key]);
  ...
  if (statements.length === 0) return Response.json({ createdCount: 0, updatedCount: 0, rejected });
  ...
  logger.info("yahrtzeit_import_committed", { userId: profile.id, createdCount, updatedCount, rejectedCount: rejected.length });
  return Response.json({ createdCount, updatedCount, rejected });
}
