...
export async function POST(request: Request) {
  ...
  if (importType === "donation") {
    ...
    // lib/import/donor-source-attribution.ts): a suggestion only, never
    // consulted to auto-resolve anything. Loaded once, read-only, from
    // the SAME bounded query pattern every other lookup table here uses.
    const knownAttributions = await env.DB.prepare(`SELECT a.source_external_id, a.source_name, a.suggested_donor_id, d.donor_code AS suggested_donor_code, d.display_name AS suggested_donor_name, a.note
      ...
      WHERE a.user_id = ? AND a.external_source = 'JL Solutions' AND d.owner_user_id = ? AND d.data_source = 'live' AND d.archived_at IS NULL`).bind(userId, userId).all<RawDonorSourceAttributionRow>();
    ...
    // Third-Party Source Attribution, payment side: a `match_donor`-style
    // decision for a payment fingerprint is verified here against the
    // SERVER's own knownAttributionsByCode -- the client's attributedDonorId
    // can only ever confirm the server's own suggestion for that exact
    // fingerprint's own source code, never select an arbitrary donor.
    const paymentActivityByFingerprint = new Map(paymentActivities.map((activity) => [activity.fingerprint, activity]));
    const verifiedPaymentAttributions = new Map<string, string>();
    for (const decision of paymentDecisions) {
      if (!decision.attributedDonorId) continue;
      const activity = paymentActivityByFingerprint.get(decision.fingerprint);
      const suggestion = activity ? knownAttributionsByCode.get(activity.externalHouseholdId.toLowerCase()) : undefined;
      if (suggestion && suggestion.suggestedDonorId === decision.attributedDonorId) verifiedPaymentAttributions.set(decision.fingerprint, decision.attributedDonorId);
    }
    const paymentAttributedDonorIds = [...new Set(verifiedPaymentAttributions.values())];
    const donorIds = [...new Set([...households.results.map((household) => household.id), ...paymentAttributedDonorIds])];
    // Attributed donors need their own household record available to
    // buildPaymentCandidates (for display name / pledge lookup) even
    // though their OWN JL code never appears in this file's rows.
    const attributedHouseholds = paymentAttributedDonorIds.length
      ? await env.DB.prepare(`SELECT id, external_id, display_name FROM donors WHERE owner_user_id = ? AND data_source = 'live' AND archived_at IS NULL AND id IN (SELECT value FROM json_each(?))`).bind(userId, JSON.stringify(paymentAttributedDonorIds)).all<MatchedHousehold & { display_name: string }>()
      : { results: [] as Array<MatchedHousehold & { display_name: string }> };
    const householdsForPayments = [...households.results, ...attributedHouseholds.results];
    ...
    // re-run at commit time using the same live query the preview used, so
    // a decision made against a stale preview can never silently bypass
    // this check. See lib/import/jl-payment-duplicate-match.ts.
    const existingCompletedGifts = donorIds.length
      ? await env.DB.prepare(EXISTING_COMPLETED_GIFTS_FOR_DUPLICATE_MATCH_SQL).bind(userId, JSON.stringify(donorIds)).all<ExistingCompletedGiftRow>()
      : { results: [] as ExistingCompletedGiftRow[] };
    const paymentCandidates = buildPaymentCandidates(paymentActivities, householdsForPayments, openPledges.results, rememberedWithLegacyGifts, existingCompletedGifts.results, knownAttributionsByCode, verifiedPaymentAttributions);
    ...
    if (allRowsRequireReview) {
      return Response.json({
        ...
        donation: { newActivities: 0, updatedPledges: 0, unchanged: 0, unknownHousehold: 0, needsReview: genuinelyUnresolvedRows.length, nonfinancialExcluded: 0, duplicateSourceRows: 0 },
        validation,
        reviewRows,
        rejectedRows: [],
    ...
    if (!matchedActivities.length && !assignmentPlan.pledgeUpdates.length && !alreadyImported && !assignmentPlan.skippedDuplicates.length) {
    ...
    if (body.forceReprocess && !newActivities.length && !proposedUpdates.length && !assignmentPlan.pledgeUpdates.length && !assignmentPlan.skippedDuplicates.length) {
      return Response.json({
        importId: activeDuplicate!.id,
        fileName,
        completedAt: new Date().toISOString(),
        ...
        noChangesMade: true,
        imported: { donors: 0, gifts: 0, interactions: 0, reminders: 0 },
        donation: { newActivities: 0, updatedPledges: 0, unchanged: alreadyImported, unknownHousehold: 0, needsReview: genuinelyUnresolvedRows.length, nonfinancialExcluded: match.nonfinancial, duplicateSourceRows: donationPreview.duplicateRows.length },
        validation,
        reviewRows,
        rejectedRows,
    ...
    // "Skip -- already recorded" (Giving Import Reconciliation): the
    // source row makes no giving_activities mutation of any kind, but its
    // resolution -- and the existing record it was matched against, when
    // one was found -- is still recorded here so "what happened to this
    // imported payment" is answerable from the database, not only from
    // rendered UI text (see lib/import/jl-payment-assignment.ts's
    // PlannedSkippedDuplicate and lib/import/donation-rollback.ts's own
    // explicit no-op handling of this change_type).
    const skippedDuplicateChangeRows = assignmentPlan.skippedDuplicates.map((skip) => ({ importId, fingerprint: skip.fingerprint, changeType: "skipped_duplicate", previousJson: JSON.stringify({ matchedActivityId: skip.matchedActivityId, matchConfidence: skip.matchConfidence }), now }));
    const changeRows = [...newActivities.map((activity) => ({ importId, fingerprint: activity.fingerprint, changeType: "insert", previousJson: crossImportAuditByFingerprint.get(activity.fingerprint) ?? rejectionAuditByFingerprint.get(activity.fingerprint) ?? dateAuditByFingerprint.get(activity.fingerprint) ?? null, now })), ...proposedUpdates.map((activity) => ({ importId, fingerprint: activity.fingerprint, changeType: "update", previousJson: JSON.stringify(priorByFingerprint.get(activity.fingerprint)), now })), ...assignmentPlan.pledgeUpdates.map((update) => ({ importId, fingerprint: update.source_fingerprint, changeType: "update", previousJson: JSON.stringify({ source_fingerprint: update.source_fingerprint, paid_cents: update.paid_cents, balance_cents: update.balance_cents, category: update.category, source_snapshot: update.source_snapshot }), now })), ...skippedDuplicateChangeRows];
    ...
    // Third-Party Source Attribution: `knownAttribution` is only ever
    // present on a candidate whose donor was resolved via attribution
    // (see buildPaymentCandidates) -- recording it here is what lets
    // "JL payer: Price Waterhouse Foundation (22297) / FOS attributed
    // donor: Eitan Pfeiffer (48637)" be reconstructed later directly from
    // this audit row, since an apply_to_pledge outcome never creates a
    // new giving_activities row of its own. Null for every ordinary
    // payment. Looked up by the SAME candidate.fingerprint a multi-pledge
    // split's own "parent" marker row already uses (see planPaymentAssignments'
    // own comment) -- an individual per-pledge allocation row (a
    // genuine 2+-way split) carries a synthetic fingerprint this lookup
    // will not find, so it is left null on those specific rows; the
    // parent row for the same payment still carries it.
    const assignmentAuditRows = assignmentPlan.assignments.map((assignment) => {
      const candidate = candidateByFingerprint.get(assignment.fingerprint);
      return { id: crypto.randomUUID(), userId, importId, paymentFingerprint: assignment.fingerprint, donorId: assignment.donorId, pledgeId: assignment.pledgeId, decisionType: assignment.decisionType, paymentCents: assignment.paymentCents, appliedCents: assignment.appliedCents, newGiftCents: assignment.newGiftCents, overpaymentAction: assignment.overpaymentAction, previousPaidCents: assignment.previousPaidCents, nextPaidCents: assignment.nextPaidCents, previousBalanceCents: assignment.previousBalanceCents, nextBalanceCents: assignment.nextBalanceCents, previousStatus: assignment.previousStatus, nextStatus: assignment.nextStatus, paymentDate: candidateByFingerprint.get(assignment.fingerprint)?.paymentDate ?? now, remainingBalanceCents: assignment.nextBalanceCents, sourceExternalId: candidate?.attributedDonorId ? candidate.knownAttribution?.sourceExternalId ?? null : null, sourceName: candidate?.attributedDonorId ? candidate.knownAttribution?.sourceName ?? null : null, now };
    });
