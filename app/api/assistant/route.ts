...
export async function POST(request: Request) {
  ...
  try {
    ...
    const brief = await loadWorkspaceBrief(profile.id, profile.timezone, mode);
    ...
    const [donor, interactions, recommendations] = primaryId ? await Promise.all([
      ...
      env.DB.prepare(`SELECT id, summary, occurred_at FROM interactions WHERE donor_id = ? ${mode === "demo" ? "" : "AND user_id = ?"} ORDER BY occurred_at DESC LIMIT 1`).bind(...(mode === "demo" ? [primaryId] : [primaryId, profile.id])).all<InteractionRow>(),
