...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [donor, interactions, recommendations] = primaryId ? await Promise.all([
      ...
      env.DB.prepare(`SELECT id, summary, occurred_at FROM interactions WHERE donor_id = ? ${mode === "demo" ? "" : "AND user_id = ?"} AND occurred_at <= ? AND source NOT LIKE 'capture-scheduled:%' AND source NOT LIKE 'cancelled:%' AND source NOT LIKE 'archived:%' AND occurred_at <= created_at ORDER BY occurred_at DESC LIMIT 1`).bind(...(mode === "demo" ? [primaryId, now] : [primaryId, profile.id, now])).all<InteractionRow>(),
