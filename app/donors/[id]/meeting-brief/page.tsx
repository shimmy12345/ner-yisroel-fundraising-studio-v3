...
import { askLine, familyDateLine } from "../../../../lib/relationships/meeting-brief-model";
...
export default async function MeetingBriefPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; origin?: string }> }) {
  ...
  return <AppShell active="donors"><main className="meeting-brief-page">
    ...
    <div className="meeting-brief-essential-grid">
      ...
      {/* First-class, independent of Suggested Action -- an open ask
          appears here whenever one exists, regardless of whether it also
          happens to win the single Suggested Action slot above. Never
          rendered empty (no section at all when the donor has no open
          ask), matching FAMILY CONTEXT's own conditional-render
          convention below, not OPEN COMMITMENTS' always-render-with-
          empty-state one -- this is deliberately the quieter of the two
          patterns already used on this page. */}
      {brief.openAsks.length > 0 && <section className="meeting-brief-card"><div className="meeting-card-heading"><p className="eyebrow">{brief.openAsks.length > 1 ? "OPEN ASKS" : "OPEN ASK"}</p><h2>{brief.openAsks.length > 1 ? "Pending asks" : "Pending ask"}</h2></div><div className="meeting-commitment-list">{brief.openAsks.map((item) => <article key={item.id}><strong>{askLine(item, (epoch) => date(epoch, profile.timezone))}</strong>{item.followUpDueAt !== null && <span>Follow-up: {date(item.followUpDueAt, profile.timezone)}</span>}</article>)}</div></section>}
