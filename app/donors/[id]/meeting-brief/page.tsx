...
export default async function MeetingBriefPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; origin?: string }> }) {
  ...
  return <AppShell active="donors"><main className="meeting-brief-page">
    ...
    <div className="meeting-brief-essential-grid">
      ...
      {brief.familyYahrtzeits.length > 0 && <section className="meeting-brief-card"><div className="meeting-card-heading"><p className="eyebrow">FAMILY CONTEXT</p><h2>Yahrtzeits</h2></div><ul className="meeting-topic-list">{brief.familyYahrtzeits.map((item) => <li key={`${item.relationship}-${item.deceasedNameEnglish}`}>{item.relationship}&apos;s yahrtzeit is {item.hebrewLabel}; next occurrence {item.nextOccurrenceLabel}.</li>)}</ul></section>}
