...
type DonorOption = { id: string; name: string; primaryFirstName: string | null; lastName: string | null; code: string | null };
...
export function MergeExperience({ current, other, donors }: { current: MergeView; other: MergeView | null; donors: DonorOption[] }) {
  ...
  return <main className="merge-workspace">
    ...
    <section className="merge-picker"><label><span>Compare with</span><select value={selectedOther} onChange={(event) => setSelectedOther(event.target.value)}><option value="">Choose another donor</option>{donors.map((donor) => { const code = numericDonorCode({ donorCode: donor.code }); return <option value={donor.id} key={donor.id}>{donor.name}{code ? ` · ${code}` : ""}</option>; })}</select></label><button type="button" disabled={!selectedOther} onClick={openComparison}>Compare records</button></section>
    {other && <>
      <section className="merge-survivor"><h2>Which donor should survive?</h2>{[current, other].map((donor) => { const code = numericDonorCode({ donorCode: donor.code }); return <label key={donor.id}><input type="radio" name="survivor" checked={survivorId === donor.id} onChange={() => { setSurvivorId(donor.id); setConfirmed(false); }} /><span className="mini-avatar">{donorInitials({ displayName: donor.name, primaryFirstName: donor.primaryFirstName, lastName: donor.lastName })}</span><span><strong>{donor.name}</strong>{code && <span className="donor-code">{code}</span>}<small>{code ? "JL Solutions" : "Manual donor"}</small></span></label>; })}</section>
      ...
      <section className="merge-history"><h2>History that will be preserved</h2><div>{[current, other].map((donor) => <article key={donor.id}><span className="mini-avatar">{donorInitials({ displayName: donor.name, primaryFirstName: donor.primaryFirstName, lastName: donor.lastName })}</span><div><strong>{donor.name}</strong>{numericDonorCode({ donorCode: donor.code }) && <span className="donor-code">{numericDonorCode({ donorCode: donor.code })}</span>}</div>{Object.entries(donor.counts).map(([label, count]) => <span key={label}>{label}: {count}</span>)}</article>)}</div><p>All gifts, pledges, interactions, meetings, reminders, notes, and audit records will move to the survivor. The duplicate will be archived and old links will redirect.</p></section>
