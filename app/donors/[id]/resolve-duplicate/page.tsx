...
export default async function ResolveDuplicatePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ otherId?: string }> }) {
  ...
  const candidates = await env.DB.prepare("SELECT id,display_name,COALESCE(external_id,donor_code) code FROM donors WHERE owner_user_id=? AND data_source='live' AND archived_at IS NULL AND id<>? ORDER BY last_name COLLATE NOCASE,display_name COLLATE NOCASE").bind(profile.id,id).all<{id:string;display_name:string;code:string|null}>();
  ...
  return <AppShell active="donors"><MergeExperience current={view(current,currentCounts!)} other={other && otherCounts ? view(other,otherCounts) : null} donors={candidates.results.map((row)=>({id:row.id,name:row.display_name,code:row.code}))} /></AppShell>;
}
