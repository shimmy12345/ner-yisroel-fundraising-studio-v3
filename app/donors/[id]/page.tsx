...
import { buildDonorRecommendation } from "../../../lib/relationships/recommendation-rank";
...
import { YahrtzeitManagement, type YahrtzeitListItem } from "./YahrtzeitManagement";
...
type Interaction = { id: string; type: string; occurred_at: number; occurred_at_date_only: number; summary: string; source: string; created_at: number; status_changed_at: number | null };
...
export default async function DonorPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; origin?: string }> }) {
  ...
  const yahrtzeitRows = mode === "live"
    ...
    : [];
  const yahrtzeitItems: YahrtzeitListItem[] = yahrtzeitRows.map((row) => ({
    id: row.id,
  ...
  const recommendationEvidence = buildRecommendationEvidence({
    ...
    yahrtzeits: yahrtzeitItems.map((item) => ({ deceasedNameEnglish: item.deceasedNameEnglish, deceasedNameHebrew: item.deceasedNameHebrew, relationship: item.relationship, hebrewMonth: item.hebrewMonth, hebrewDay: item.hebrewDay })),
  ...
  return <AppShell active="donors"><div className="donor-breadcrumb"><a href="/">Workspace</a><span>/</span><a href={donorDirectoryHref}>Donors</a><span>/</span><strong>{donor.display_name}</strong></div>
    ...
    <div className="relationship-grid"><main className="relationship-main">
      ...
      {mode === "live" && <section className="story-card yahrtzeit-card"><div className="card-heading"><div><p className="eyebrow">YAHRTZEITS</p><h2>Family remembrance dates</h2><p>Background context, not a logged interaction. Never counted as contact.</p></div></div><YahrtzeitManagement donorId={id} timezone={profile.timezone} items={yahrtzeitItems} /></section>}
