...
export default async function DonorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (id !== "elena-chen") {
    ...
    if (household) {
      ...
      const activities: GivingActivity[] = activityRows.results.map((row) => ({ fingerprint: row.source_fingerprint, externalHouseholdId: row.external_household_id, sourceName: "", activityDate: row.activity_date, committedCents: row.committed_cents, paidCents: row.paid_cents, balanceCents: row.balance_cents, itemType: row.item_type ?? "", description: row.description ?? "", sourceCampaign: row.source_campaign ?? "", category: row.category, suspiciousDate: false, reviewReason: null, sourceValues: {} }));
