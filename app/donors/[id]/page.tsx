...
export default async function DonorPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ from?: string; origin?: string }> }) {
  ...
  const paidFromActivities = countedActivities.filter((item) => (item.paid_cents ?? 0) > 0 && item.activity_date !== null).map((item) => ({ amountCents: item.paid_cents!, occurredAt: item.activity_date!, campaign: item.source_campaign, description: item.description || item.item_type }));
  const paidFromLegacy = legacyGifts.map((gift) => ({ amountCents: gift.amount_cents, occurredAt: gift.received_at, campaign: gift.fund as string | null, description: null as string | null }));
