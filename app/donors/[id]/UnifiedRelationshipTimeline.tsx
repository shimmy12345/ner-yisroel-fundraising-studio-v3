...
export function UnifiedRelationshipTimeline({ giving, legacyGifts, payments, interactions, reminders, donors, timezone, live, now }: {
  ...
  return <>
    ...
    {visible.length === 0 ? <div className="unified-timeline-empty"><strong>No {selectedLabel.toLowerCase()} recorded</strong><p>Choose another filter to continue reviewing this donor&apos;s relationship history.</p></div> : <div className="timeline-list unified-timeline-list">{visible.map((item) => {
      ...
      if (item.kind === "payment") return <article className="timeline-item unified-timeline-item completed pledge-payment-event" key={item.key}><time><strong>{financialDateLabel(item.eventAt)}</strong></time><span className="timeline-dot gift">$</span><div className="timeline-content"><div><h3>Payment applied to pledge</h3><span className="event-type">Payment</span>{item.payment.pledge_campaign && <span className="event-campaign">{item.payment.pledge_campaign}</span>}<span className="timeline-status completed">Completed</span></div><p>{money(item.payment.applied_cents)} paid · {money(item.payment.remaining_balance_cents ?? 0)} remaining</p>{item.linkedPledgeExists ? <a className="timeline-linked-record" href={`#pledge-${encodeURIComponent(item.payment.pledge_activity_id)}`}>{item.payment.pledge_description ? `Linked pledge: ${item.payment.pledge_description}` : "View linked pledge"}</a> : <small className="timeline-link-unavailable">Linked pledge is unavailable</small>}</div></article>;
