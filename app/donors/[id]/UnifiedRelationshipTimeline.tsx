...
// Monday.com supplies a calendar date only -- occurredAt/dueAt is anchored
// at UTC noon to avoid a timezone day-shift, which is not evidence of a
// real time of day. occurred_at_date_only/due_at_date_only is the only
// signal for this; never infer it from the clock value itself.
const eventDate = (epoch: number, timezone: string, dateOnlyFlag: number | undefined) => dateOnlyFlag ? dateOnly(epoch, timezone) : dateTime(epoch, timezone);
...
export function UnifiedRelationshipTimeline({ giving, legacyGifts, payments, interactions, reminders, donors, timezone, live, now, acknowledgments = {} }: {
  ...
  return <>
    ...
    {visible.length === 0 ? <div className="unified-timeline-empty"><strong>No {selectedLabel.toLowerCase()} recorded</strong><p>Choose another filter to continue reviewing this donor&apos;s relationship history.</p></div> : <div className="timeline-list unified-timeline-list">{visibleSlice.map((item) => {
      ...
      return <article className={`timeline-item unified-timeline-item ${item.status}`} key={item.key}><time><strong>{eventDate(item.eventAt, timezone, activity.occurred_at_date_only)}</strong></time><span className="timeline-dot">•</span><div className="timeline-content"><div><h3>{timelineTitle}</h3><span className="event-type">{typeLabel}</span><span className={`timeline-status ${item.status}`}>{STATUS_LABELS[item.status]}</span></div>{item.plannedAt && item.status === "completed" && <small className="timeline-planned-date">Originally planned for {dateTime(item.plannedAt, timezone)}</small>}<p>{timelineNote}</p>{live && scheduled && <a className="timeline-outcome-link" href={`/interactions/${encodeURIComponent(activity.id)}/outcome`}>Log Outcome</a>}{live && item.status === "cancelled" && <a className="timeline-outcome-link secondary" href={`/interactions/${encodeURIComponent(activity.id)}/outcome`}>Edit or reopen</a>}{live && item.status !== "cancelled" && <ActivityActions activityId={activity.id} editHref={`/interactions/${encodeURIComponent(activity.id)}/edit`} scheduled={scheduled} canCancel={scheduled && activity.occurred_at > now} />}</div></article>;
    })}</div>}
