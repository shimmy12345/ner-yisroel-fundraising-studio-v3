  ...
  return <>
    ...
    {visible.length === 0 ? <div className="unified-timeline-empty"><strong>No {selectedLabel.toLowerCase()} recorded</strong><p>Choose another filter to continue reviewing this donor&apos;s relationship history.</p></div> : <div className="timeline-list unified-timeline-list">{visible.map((item) => {
      if (item.kind === "giving") {
        ...
        return <article id={`pledge-${activity.id}`} className={`timeline-item unified-timeline-item ${item.status}`} key={item.key}><time><strong>{activity.activity_date ? dateTime(item.eventAt, timezone) : "Date not recorded"}</strong></time><span className="timeline-dot gift">$</span><div className="timeline-content"><div><h3>{title}</h3><span className="event-type">{item.filter === "pledges" ? "Pledge" : "Gift"}</span><span className={`timeline-status ${item.status}`}>{STATUS_LABELS[item.status]}</span></div><p>{money(amount)} committed{(activity.paid_cents ?? 0) > 0 ? ` · ${money(activity.paid_cents ?? 0)} paid` : ""}{(activity.balance_cents ?? 0) > 0 ? ` · ${money(activity.balance_cents ?? 0)} open` : ""}</p>{live && <GivingRecordActions activity={{ id: activity.id, donorId: activity.donor_id, externalSource: activity.external_source, workspaceStatus: activity.workspace_status, privateNote: activity.private_note, updatedAt: activity.updated_at }} donors={donors} />}</div></article>;
      }
      if (item.kind === "legacy-gift") return <article className="timeline-item unified-timeline-item completed" key={item.key}><time><strong>{dateTime(item.eventAt, timezone)}</strong></time><span className="timeline-dot gift">$</span><div className="timeline-content"><div><h3>{item.gift.fund || "Gift"}</h3><span className="event-type">Gift</span><span className="timeline-status completed">Completed</span></div><p>{money(item.gift.amount_cents)} paid</p></div></article>;
      if (item.kind === "payment") return <article className="timeline-item unified-timeline-item completed pledge-payment-event" key={item.key}><time><strong>{dateTime(item.eventAt, timezone)}</strong></time><span className="timeline-dot gift">$</span><div className="timeline-content"><div><h3>Payment applied to pledge</h3><span className="event-type">Payment</span><span className="timeline-status completed">Completed</span></div><p>{money(item.payment.applied_cents)} paid · {money(item.payment.remaining_balance_cents ?? 0)} remaining</p>{item.linkedPledgeExists ? <a className="timeline-linked-record" href={`#pledge-${encodeURIComponent(item.payment.pledge_activity_id)}`}>{item.payment.pledge_description ? `Linked pledge: ${item.payment.pledge_description}` : "View linked pledge"}</a> : <small className="timeline-link-unavailable">Linked pledge is unavailable</small>}</div></article>;
      ...
      const [subject, ...notes] = activity.summary.split("\n");
      ...
      return <article className={`timeline-item unified-timeline-item ${item.status}`} key={item.key}><time><strong>{dateTime(item.eventAt, timezone)}</strong></time><span className="timeline-dot">•</span><div className="timeline-content"><div><h3>{subject || `${activity.type} activity`}</h3><span className="event-type">{typeLabel}</span><span className={`timeline-status ${item.status}`}>{STATUS_LABELS[item.status]}</span></div>{item.plannedAt && item.status === "completed" && <small className="timeline-planned-date">Originally planned for {dateTime(item.plannedAt, timezone)}</small>}<p>{notes.join("\n") || "No additional notes recorded."}</p>{live && scheduled && <a className="timeline-outcome-link" href={`/interactions/${encodeURIComponent(activity.id)}/outcome`}>Log Outcome</a>}{live && item.status === "cancelled" && <a className="timeline-outcome-link secondary" href={`/interactions/${encodeURIComponent(activity.id)}/outcome`}>Edit or reopen</a>}{live && item.status !== "cancelled" && <ActivityActions activityId={activity.id} editHref={`/interactions/${encodeURIComponent(activity.id)}/edit`} scheduled={scheduled} canCancel={scheduled && activity.occurred_at > now} />}</div></article>;
    })}</div>}
  </>;
}
