...
function MeetingCard({ meeting, live }: { meeting: WorkspaceMeeting; live: boolean }) {
  return <article className="meeting">
...
export default async function TodayPage() {
  ...
  return <AppShell active="today">
    ...
    <div className="dashboard-grid">
      <main>
        <div className="section-title"><div><h2>Top priorities</h2><p>Derived from live reminders, gifts, pledges, and relationship activity</p></div></div>
        {data.priorities.length ? <div className="priority-list">{data.priorities.map((item, index) => <PriorityCard key={item.donorId} priority={item} index={index} />)}</div> : <section className="directory-empty"><h2>No priorities yet</h2><p>Your workspace has no due reminders, open pledges, recent gifts, or lapsed relationships requiring attention.</p><a href="/capture">Log an interaction</a></section>}
      </main>
      <aside className="right-rail">
        <section className="rail-card">
          <div className="rail-heading"><div><span className="rail-icon">□</span><h2>Upcoming meetings</h2></div><span className="count">{data.meetings.length}</span></div>
          {data.meetings.length ? data.meetings.map((item) => <MeetingCard meeting={item} live={mode === "live"} key={`${item.donorId}-${item.time}`} />) : <p className="empty-copy">No upcoming meetings are recorded.</p>}
        </section>
        <section className="rail-card">
          <div className="rail-heading"><div><span className="rail-icon gift-icon">◇</span><h2>Recent gifts</h2></div><span className="count">{data.gifts.length}</span></div>
          {data.gifts.length ? data.gifts.map((gift) => <a className="gift-row" key={gift.id} href={`/donors/${encodeURIComponent(gift.donorId)}`}><div className="mini-avatar">{gift.initials}</div><div><h3>{gift.name}</h3><p>{gift.detail}</p></div><strong>{gift.amount}</strong></a>) : <p className="empty-copy">No gifts were recorded in the last 30 days.</p>}
        </section>
        <section className="rail-card compact-card">
          <div className="rail-heading"><div><span className="rail-icon">↗</span><h2>Quick actions</h2></div></div>
          <div className="quick-grid"><a href="/capture"><span>＋</span>Log interaction</a><a href="/assistant"><span>✦</span>Ask Assistant</a><a href="/donors"><span>⌕</span>Find donor</a><a href="/onboarding/import"><span>⇧</span>Import data</a></div>
        </section>
      </aside>
    </div>
  </AppShell>;
