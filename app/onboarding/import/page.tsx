...
export default async function ImportPage() {
  ...
  const history = parsedImports.map(({ row, report }) => {
    ...
    return { id: row.id, fileName: row.file_name, status: row.status, canUndo: row.id === latestCompletedDonationId, completedAt: row.completed_at ? new Date(row.completed_at * 1000).toISOString() : null,
