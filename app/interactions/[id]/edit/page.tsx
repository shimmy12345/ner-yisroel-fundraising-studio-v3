...
export default async function EditActivityPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ returnTo?: string }> }) {
  ...
  const [activity, reminder, donors] = await Promise.all([
    ...
    env.DB.prepare(`SELECT id, display_name, primary_first_name, last_name, spouse, spouse_first_name, donor_code, external_id, email, phone, home_phone, alternate_mobile_phone FROM donors
  ...
  return <AppShell active="donors"><CaptureExperience donors={donors.results.map((item) => ({
    id: item.id, name: item.display_name, primaryFirstName: item.primary_first_name, lastName: item.last_name, spouse: item.spouse || item.spouse_first_name,
