...
// 25 as of 0024_donor_historical_context.sql — adds the donor_historical_context
// table, so PRODUCTION_BASELINE_HASH changed again (PRODUCTION_BASELINE_LEVEL
// stays "0019": that label identifies the single bootstrap file's origin,
// not its current contents).
export const PRODUCTION_BASELINE_VERIFIED = PRODUCTION_BASELINE_LEVEL === "0019" && /^[a-f0-9]{64}$/.test(PRODUCTION_BASELINE_HASH) && PRODUCTION_BASELINE_SOURCE_MIGRATIONS.length === 25;
