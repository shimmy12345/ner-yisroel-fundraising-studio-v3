...
export async function getRelationshipUpdates(donorId: string, userId: string): Promise<RelationshipUpdates> {
  try {
    const [donor, interaction, recommendation] = await Promise.all([
      ...
      env.DB.prepare("SELECT id, summary, source, occurred_at FROM interactions WHERE donor_id = ? AND user_id = ? AND occurred_at <= ? AND source NOT LIKE 'cancelled:%' AND source NOT LIKE 'archived:%' AND (source LIKE 'capture-completed:%' OR (source NOT LIKE 'capture-scheduled:%' AND occurred_at <= created_at)) ORDER BY occurred_at DESC LIMIT 1")
