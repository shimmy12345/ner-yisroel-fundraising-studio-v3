...
export function matchJlDonationActivities(preview: DonationPreview, households: MatchedHousehold[], existing: ExistingGivingActivity[]) {
  ...
  for (const activity of preview.activities) {
    if (activity.category === "needs_review") { needsReview += 1; continue; }
    if (activity.category === "nonfinancial_entry") { nonfinancial += 1; continue; }
    const donorId = householdByCode.get(activity.externalHouseholdId.toLowerCase());
    if (!donorId) { unknownHousehold += 1; continue; }
  ...
  return { matched, newActivities, proposedUpdates, alreadyImported, unknownHousehold, needsReview, nonfinancial };
}
