...
async function api(path, options = {}) {
  ...
  let data;
  try { data = JSON.parse(text); } catch { throw new Error(`Server returned an unexpected response (${res.status}).`); }
...
async function boot() {
  ...
  supabase.auth.onAuthStateChange((_event, nextSession) => { session = nextSession; renderAuth(); });
  renderAuth();
}
function renderAuth() {
...
$$('[data-auth]').forEach(button => button.onclick = () => {
  ...
  $$('[data-auth]').forEach(x => x.classList.toggle('active', x === button));
...
$('generate').onclick = async () => {
  ...
  } catch (error) { $('resultEmpty').textContent = error.message; }
...
$('saveFavorite').onclick = async () => {
  ...
  const { error } = await supabase.from('generations').update({ favorite: true }).eq('id', currentGeneration.id).eq('user_id', session.user.id);
  if (error) toast(error.message); else toast('Added to favorites');
};

async function loadKnowledge() {
  const { data, error } = await supabase.from('knowledge_documents').select('*').eq('user_id', session.user.id).order('updated_at', { ascending: false });
...
function previewKnowledge(row) {
  ...
  $('modalBody').innerHTML = `<div class="document-meta"><strong>${esc(row.file_name || row.source_type)}</strong><span>${row.file_size ? formatBytes(row.file_size) : ''}</span></div><pre class="document-preview">${esc(row.content || '')}</pre>`;
...
async function toggleKnowledgeFavorite(row) {
  const { error } = await supabase.from('knowledge_documents').update({ favorite: !row.favorite }).eq('id', row.id).eq('user_id', session.user.id);
  if (error) return toast(error.message);
...
uploadZone.onclick = event => { if (!event.target.closest('input') && !event.target.closest('button')) $('knowledgeFiles').click(); };
uploadZone.onkeydown = event => { if (event.key === 'Enter' || event.key === ' ') $('knowledgeFiles').click(); };
...
async function uploadKnowledgeFiles(files) {
  ...
  const allowed = /\.(pdf|doc|docx|txt|md|csv|json|html?|rtf|xlsx?|pptx?)$/i;
  ...
  for (const file of files) {
    ...
    item.innerHTML = `<div><strong>${esc(file.name)}</strong><span>${formatBytes(file.size)}</span></div><span class="upload-state">Waiting</span>`;
    queue.appendChild(item);
    const state = item.querySelector('.upload-state');
    try {
      if (!allowed.test(file.name)) throw new Error('Unsupported file type');
      if (file.size > 25 * 1024 * 1024) throw new Error('Larger than 25 MB');
      state.textContent = 'Checking…';
      const checksum = await sha256(file);
      const { data: duplicate, error: duplicateError } = await supabase.from('knowledge_documents').select('id,title').eq('user_id', session.user.id).eq('checksum', checksum).maybeSingle();
      ...
      const { error: uploadError } = await supabase.storage.from('knowledge-files').upload(storagePath, file, { contentType: file.type || 'application/octet-stream', upsert: false });
  ...
  await loadKnowledge();
  toast('Upload processing complete');
}

async function loadDonors() {
  const { data, error } = await supabase.from('donors').select('*').eq('user_id', session.user.id).order('name');
  if (error) return toast(error.message);
  ...
  try {
    if (modalType === 'knowledge') {
      ...
      const query = modalRecord
        ? supabase.from('knowledge_documents').update(row).eq('id', modalRecord.id).eq('user_id', session.user.id)
      ...
      const query = modalRecord
        ? supabase.from('donors').update(row).eq('id', modalRecord.id).eq('user_id', session.user.id)
        : supabase.from('donors').insert(row);
      const { error } = await query;
      if (error) throw error;
      await loadDonors();
    }
...
$('deleteModal').onclick = async () => {
  ...
  if (modalType === 'knowledge') {
    if (modalRecord.storage_path) {
      const { error: storageError } = await supabase.storage.from('knowledge-files').remove([modalRecord.storage_path]);
      if (storageError) return toast(storageError.message);
    }
    const { error } = await supabase.from('knowledge_documents').delete().eq('id', modalRecord.id).eq('user_id', session.user.id);
    if (error) return toast(error.message);
    await loadKnowledge();
  } else {
    const { error } = await supabase.from('donors').delete().eq('id', modalRecord.id).eq('user_id', session.user.id);
    if (error) return toast(error.message);
    await loadDonors();
  }
  $('modal').classList.add('hidden');
...
async function loadHistory() {
  const { data, error } = await supabase.from('generations').select('*').eq('user_id', session.user.id).order('created_at', { ascending: false }).limit(100);
  if (error) return toast(error.message);
